...
def collect_driver_usage(run_fn, packages=None):
    ...
    EZ MA CSAK EGY VÉKONY RÉTEG a `app/driverusage_core.py` közös magja fölött
    (2026-09-17, explicit user decision: a Driverek nézet és a fix törlési előnézete
    UGYANAZT a felderítést használja, két implementáció nélkül). A régi, saját
    `Win32_PnPSignedDriver`-lekérdezés ELTÁVOLÍTVA - nem azért, mert rossz volt, hanem
    mert a magban lévő registry-alapú út bizonyítottan ugyanazt adja a jelenlévő
    eszközökre (mérve: pontosan ugyanaz a 64 INF, 0 eltéréssel mindkét irányban),
    csak gyorsabban, ÉS emellett a nem jelenlévő eszközöket meg a futó
    kernel-szolgáltatásokat is látja - amiket a `Win32_PnPSignedDriver` elvből nem tud.

    Visszatérés: {publikált_inf_kisbetűvel: [eszköznevek]} - változatlan alak, mert a
    lánc három hívója (törlés előtti pillanatkép, "nem jött vissza" jelentés, törlési
    előnézet) szemantikailag is a JELENLÉVŐ eszközökre kérdez. Hibánál üres dict."""
    from app.driverusage_core import collect_package_usage, present_device_map
    try:
        return present_device_map(collect_package_usage(run_fn, packages))
    except Exception as e:
